...
plt_raw_normalized <-  df_gluc_data_model %>% group_by(stay_id) %>% 
  ...
  #scale_y_continuous(name='Glucose (mmol/L) (change from patient average)') + 
                     #sec.axis = sec_axis(~ . *18.0182, name='Glucose (mg/dL) (change from patient average)')) #+ 
  #ggtitle("Per-patient normalized average\nglucose levels by time of day")
plt_raw_normalized
...
## Intercept model
formula_model_intercept <- as.formula(paste(dependent_variable, "~", "1 +", random_variable))
lmemodel_gluc_time_model_intercept <- lmer(formula_model_intercept, data=df_gluc_data_model, REML=TRUE) # NB: REML=TRUE is the default setting

## Model 1 (Patient-level variables)
formula_model_1 <- as.formula(paste(dependent_variable, "~", paste(patient_level_variables, collapse=" + "), " + ", random_variable))
lmemodel_gluc_time_model_1 <- lmer(formula_model_1, data=df_gluc_data_model, REML=TRUE)

## Model 2 (Patient-level + sample-level variables)
formula_model_2 <-  as.formula(paste(dependent_variable, "~", paste(patient_level_variables, collapse=" + "), " + ", paste(sample_level_variables, collapse=" + "), " + ", random_variable))
lmemodel_gluc_time_model_2 <- lmer(formula_model_2, data=df_gluc_data_model, REML=TRUE)

## Final model (Patient-level + sample-level + time variables)
...
lmemodel_gluc_time_model_final <- lmer(formula_model_final, data=df_gluc_data_model, REML=TRUE)
...
aov_intercept_model_1 <- anova(lmemodel_gluc_time_model_intercept, lmemodel_gluc_time_model_1, refit = TRUE) # Refitting with ML instead of REML
aov_model_1_model_2 <- anova(lmemodel_gluc_time_model_1, lmemodel_gluc_time_model_2, refit = TRUE)
aov_model_2_final_model <- anova(lmemodel_gluc_time_model_2, lmemodel_gluc_time_model_final, refit = TRUE)

## Root mean squared error 
rmse <- data.frame(intercept_model = sqrt(mean(residuals(lmemodel_gluc_time_model_intercept)^2)),
                   model_1 = sqrt(mean(residuals(lmemodel_gluc_time_model_1)^2)),
                   model_2 = sqrt(mean(residuals(lmemodel_gluc_time_model_2)^2)),
                   final_model = sqrt(mean(residuals(lmemodel_gluc_time_model_final)^2)))
...
emm_gluc_time_final_model <- data.frame(emmeans(lmemodel_gluc_time_model_final, ~time_hbinfactor, rg.limit = 90000, weights="prop"), 
                                 model = "Linear-mixed effect model",
                                 group="all data", 
                                 npat = length(unique(df_gluc_data_model$stay_id)),
                                 nmeas = nrow(df_gluc_data_model))

plt_gluc_time_final_emm <- ggplot(emm_gluc_time_final_model, aes(x=as.numeric(as.character(time_hbinfactor)), y=emmean, ymin=asymp.LCL, ymax=asymp.UCL))+
  theme_bw()+
  theme_plot + 
